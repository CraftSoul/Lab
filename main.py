...
class app(App):
	def __init__(self):
		...
		self.amns=Image(source='image/异议.png',pos_hint={'center_x':0.5,'center_y':0.5},size_hint=(0.5,0.5),allow_stretch=True,keep_ratio=True)
		self.cnm=SoundLoader.load('image/异议.ogg')
...
class Card(RelativeLayout):
	def __init__(self,player,name,da=1,**kwargs):
		...
		with self.canvas:
			...
			self.add_widget(Image(source=f'image/{name}.png',size_hint=(0.9,0.52),pos_hint={'x':0.05,'y':0.45},allow_stretch=True,keep_ratio=False))
...
class Player:
	def __init__(self,name,is_player,x,take,index):
		...
		if is_player:
			...
			self.head=Image(source=f'image/{name}.png',size_hint=(0.1,0.2),pos_hint={'x':x,'y':0.6},allow_stretch=True,keep_ratio=True)
